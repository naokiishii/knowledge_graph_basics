# to delete all the data on Neo4j "MATCH (n) DETACH DELETE n"
...
if IS_GRAPH_CREATED == False:
  ...
  raw_documents = WikipediaLoader(query="Shohei Ohtani").load()
  #raw_documents = WikipediaLoader(query="川崎フロンターレ", lang="ja").load()
  #print(raw_documents)
  ...
  res = kg.add_graph_documents(
      graph_documents, include_source=True, baseEntityLabel=True
  )
...
# Test it out
res = entity_chain.invoke({"question": "Who is Shohei Ohtani?"}).names
print(res)
...
print(structured_retriever("Who is Shohei Ohtani?"))
...
res_simple = chain.invoke(
    {
        "question": "When Shohei Ohtani won MVP?",
    }
)
...
res_hist = chain.invoke(
    {
        "question": "When did he won the World Series?",
        "chat_history": [
            ("Who owns Decoy Ohtani?", "Shohei Ohtani owns Decoy Ohtani."),
        ],
    }
)
